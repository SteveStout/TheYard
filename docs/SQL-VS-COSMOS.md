...
site serves one of them by default: the live site,
...
| --- | --- | --- | --- |
| What it is | A relational database: tables, rows, foreign keys, T-SQL. The schema is the contract and the engine enforces it. General Purpose serverless on the free limit when this was written, a Basic database on the same server since 14 September; one region from the compute until 20 September and in its region since. | A document database: JSON documents in containers, each container split by a partition key. The document owns its shape; the engine enforces the key, the id and the etag. Free tier, in the compute's own region until 20 September and one region from it since. | [ADR-039](https://theyard.stevenstout.biz/?doc=adr-sql-server), [ADR-059](https://theyard.stevenstout.biz/?doc=adr-second-store), [ADR-065](https://theyard.stevenstout.biz/?doc=adr-cosmos-explained) |
...
taking one round trip per operation off each side leaves them the same. Those
two numbers are a fact about where the compute was. On 20 September it moved
to the relational server's region, and the same proof read 1 to 2 ms to Azure
SQL Database and 38 to 40 ms to Azure Cosmos DB, with every store-bound row
flipped and every request unit unchanged (ADR: One plan, two sites).
