...
## Addendum, 2026-09-24: the workbench, one card at a time, the address naming it

Steve chose the third of three drawings for the tab (a workbench, one card large): "I like C by url navigation". The tab had grown to 19 cards on one scrolling page, and every card fetched when the tab opened, which is how the tab's own reads became most of the slow requests on the site (the addendum above on the typical answer). So the tab is a workbench now. **The address names the card**: `/?view=admin&card=timing` opens the timing card, `?view=admin` alone opens the first card of the first question (health), and a name that is no card opens health and says so in the rail, and the address stops naming it. **A card is a place**: opening one pushes an entry, so Back returns to the card before it, and the view's own back button still leaves the tab in one step, because each entry carries how deep into the bench it is. **The rail** down the left is the five questions in the order somebody asks them, every card under the question it answers, the open one marked, with a search box that filters it by name; it scrolls on its own when the window is shorter than it. **The strip stays** across the top and each tile is a link to the card that answers it (Health to health, Pages to pages, the typical answer to timing, visitors to the activity card, memory and request units to the machines, errors to errors, the version to the tests). **j and k** walk the rail's order wherever focus is, except in a field being typed in. **A pin** keeps a second card beside whichever one the rail opens next, so the comparison the long page gave for free is one choice away: `&pin=errors` in the address, one pin at most, unpinned from either column. Each card's own markup is what it was; what moved is which of them is on the page. The strip keeps its own reads of the page sweep and of today's people, because the cards that show those in full are not on the page unless they are open.
...
- [`src/lib/workbench.ts`](https://github.com/SteveStout/TheYard/blob/main/src/lib/workbench.ts): the workbench's addresses, React-free: the cards by slug and what an address opens, the only part of the workbench the page's first chunk carries.
- [`src/lib/bench.ts`](https://github.com/SteveStout/TheYard/blob/main/src/lib/bench.ts): the rest of it, loaded with the Admin tab's chunk: the cards under their questions, where j and k lead, what the rail's search finds, which card a tile opens and the hour at a glance.
- [`src/lib/workbench.test.ts`](https://github.com/SteveStout/TheYard/blob/main/src/lib/workbench.test.ts): every card once, a wrong name, the walk round the rail, and every tile's card.
...
- [`src/components/admin/TrafficCard.tsx`](https://github.com/SteveStout/TheYard/blob/main/src/components/admin/TrafficCard.tsx) and [`src/components/admin/ProofCard.tsx`](https://github.com/SteveStout/TheYard/blob/main/src/components/admin/ProofCard.tsx): the traffic card and the proof's bars; [`src/components/AdminPanel.tsx`](https://github.com/SteveStout/TheYard/blob/main/src/components/AdminPanel.tsx): the window both cards share.
