...
- [`tests/e2e/fonts.spec.ts`](https://github.com/SteveStout/TheYard/blob/main/tests/e2e/fonts.spec.ts): no request leaves for a font host, and IBM Plex Sans is the one face the page paints with.
...
**The page.** A new section in the sidebar, Style, between Diagrams and Hosting, with one document:
[Colour and style](https://theyard.stevenstout.biz/?doc=color-style), `docs/COLOR-STYLE.md`. One
entry a colour, with its job, where it must not go and its measured contrast; then the grounds, the
status colours, the chart order, the glass look and the rules.

**The swatches are drawn from the token sheet and are never a picture.** A picture of swatches is
wrong the day a token changes. A `swatches` fence in a served document lists tokens, one a line,
and the renderer turns the fence into a sheet (`src/lib/swatches.ts`): each chip is painted with the
token ITSELF, `var(--token)`, so it is whatever the page is running on, and beside it are the value
the sheet states and its contrast on white and on the page grey, computed from the sheet's text by
the same arithmetic the token test uses (`src/lib/contrast.ts`, which both now import). The header
gradient is a wide sample of its one token. A token a fence names and the sheet does not define is
drawn as a gap that says so. It costs the inventory nothing: the sheet's text rides in the chunk
that already carries the markdown renderer, which is fetched when the first document is opened.

| Option for the swatches | Why it was or was not taken |
| --- | --- |
| A picture, regenerated by a script | Stale between regenerations, and a second artefact to keep true |
| An endpoint that reads `tokens.css` and serves the colours | C# and a request for something the page already has |
| **A fence the renderer draws from the sheet** | No endpoint, no picture, no request; the chip is the token |

**Two tokens were added so that no component carries a raw colour.** `--color-warning-border`,
`#f5d48a`, is the hairline round a notice on the warning's soft ground, which had been a hex code in
one stylesheet; it is a mark and not text. `--scrim` is what dims the page behind a dialog and the
drawer, the brown-gray at 0.45, which had been two `rgba(` values a twentieth apart in two
stylesheets and is one now. A white in the error boundary's stylesheet became `--color-on-accent`.

**The rules, held by the gate** (`api/TheYard.Tests/StyleRulesTests.cs`, each a row in the table of
rules a change has to pass). Each fails with a sentence that says what to do.

1. No raw colour, hex, `rgb(` or `hsl(`, in any stylesheet or component outside the token sheet.
   The allow-list is empty, and an exception goes on it with its reason.
2. Every hex on the Colour and style page is the value of a token, and every colour token is on the
   page. This is the test that keeps the page true.
3. Every contrast figure the page states is the figure the tokens give, to the hundredth, and clears
   the bar its pairing needs, 4.5 for text and 3.0 for marks; a figure on the page that no pairing
   gives fails too, because it is a figure nobody checks. `tokens.test.ts` still measures every
   pairing the site makes.
4. No series token has a status colour's value, and a chart's line takes a status tone only for
   server errors.
5. The gold tokens are used only by the header, the brand mark and the named trim, by file, with
   what they trim there; never a chart's line, a tile or a ring.
6. `--gradient-header` is defined once, in the token sheet, and every header bar that paints the
   header colour paints the gradient over it.
7. Nothing that holds a word or an image is faded, and a quiet word is never on the bare ground:
   those are held against the rendered page by `glass.spec.ts`, and the seventh test here holds that
   they still are.
