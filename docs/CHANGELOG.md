...
- **1.0.0.39** (2026-09-03): Style stopped being a preference: four checks now fail the build (dotnet format, Prettier, oxlint, and the typecheck that was already there), each proven to fail on bad input, and the linter's first run found a derive-do-not-store violation in the component that serves the style document (ADR-028); the twenty-nine records moved out of four topic sections into one numbered index that opens on demand (ADR-029); the first CI run of the new style job then failed on the command that had just passed locally, which turned out to be the repository claiming CRLF line endings while storing LF, true on Windows and false on the Linux runner, now fixed in .gitattributes and enforced by both formatters (ADR-028).
