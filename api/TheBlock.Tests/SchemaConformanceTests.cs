...
///
/// `api/TheBlock.Database` holds the schema as hand-written DDL and builds to a
...
/// `api/TheBlock.Database` are written in, which is one CREATE TABLE per object
...
internal sealed class Ddl
{
    ...
    {
        string root = Path.Combine(Repo.Root(), "api", "TheBlock.Database");
        var ddl = new Ddl();
