...
by a test), and seventy-eight decision records carry the trade-off and the number behind each choice. What went
...
and Best Practices in the sidebar. Seventy-eight decision records explain each choice, and the code samples in them are read from the running build
...
- **A sidebar that documents the app from inside it:** About, App Architecture, API Reference, SQL vs
  ...
  own zoomable pages, seventy-eight decision records in one numbered index, the Bicep
...
**API (523 xUnit tests, separate `TheYard.Tests` project):** one suite per onion layer.
...
**End-to-end (70 Playwright tests):** the real stack. The landing page shows 100 of
